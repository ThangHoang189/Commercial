...
const GoldPrice = () => {
  ...
  const getGold = async () => {
    const data = await (
      await fetch(
        "https://api.metalpriceapi.com/v1/latest?api_key=5477f1bffc91f8bb43e25aa56d976a50&base=XAU&currencies=USD",
        {
          method: "get",
          headers: new Headers({
  ...
  const getVNGoldPrice = async () => {
    ...
    return [
      Number.parseFloat(pnjPrice.buyPrice),
      Number.parseFloat(pnjPrice.sellPrice),
    ]
  }

  const calculateGold = async () => {
    const [goldPrice, rateExchange, goldVND] = await Promise.all([
      getGold(),
      getExchangeRate(),
      getVNGoldPrice(),
    ])
  ...
  return (
    <div>
      <label>
        Gia vang (PNJ): {goldPNJ[0].toLocaleString()} -{" "}
        {goldPNJ[1].toLocaleString()}
      </label>
      <br />
